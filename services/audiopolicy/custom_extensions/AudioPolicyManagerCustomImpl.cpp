    ...
    if (FeatureOption::MTK_AUDIO_GAIN_TABLE || FeatureOption::MTK_AUDIO_GAIN_NVRAM) {
        ...
        if ((device != AUDIO_DEVICE_NONE) && !outputDesc->isDuplicated()) {
            if ((device & outputDesc->supportedDevices().types()) == 0) {
                MTK_ALOGV("%s invalid set device [0x%x] volume to mId[%d](device support 0x%x), use [0x%x]",
...
status_t AudioPolicyManagerCustomImpl::common_extendMuteTimeForUSBFromCheckOutputForStrategy(uint32_t& maxLatency, audio_devices_t oldDevice)
{
#if defined(MTK_AUDIO)
    if (AUDIO_DEVICE_OUT_ALL_USB & oldDevice  && maxLatency * LATENCY_MUTE_FACTOR < 500) {
        maxLatency = 500 / LATENCY_MUTE_FACTOR;
        MTK_ALOGV("%s oldDevice is 0x%x  new maxLatency %d",__FUNCTION__,oldDevice, maxLatency);
    }
    return NO_ERROR;
#else
    (void) maxLatency;
...
{
    // Not only USB Phone Call use it
    //if (FeatureOption::MTK_USB_PHONECALL) {
...
{
    if (FeatureOption::MTK_AUDIO_GAIN_NVRAM) {
        MTK_ALOGS(4, "+MapVoltoCustomVol vol = %f stream = %d volmin = %d volmax = %d", vol, stream, volmin, volmax);
        ...
        if (stream == AUDIO_STREAM_PATCH) {
           return fCUSTOM_VOLUME_MAPPING_STEP-1;
        }
        ...
        vol = volume;
        MTK_ALOGS(4, " %s volume [%f] vol [%f]", __FUNCTION__, volume, vol);
        return volume;
    } else {
...
audio_io_handle_t  AudioPolicyManagerCustomImpl::multipleRecord_policySelectInput(sp<IOProfile>& profile,
    ...
    if (isSoundTrigger) {
        ...
        if (activeNonIdleOnNonSTProfileInputs.size()) {
            ...
            if ((strncmp(profile->getTagName().string(), "primary input", strlen("primary input")) == 0)
                && (!(activeNonIdleOnNonSTProfileInputs[0]->flagsToOpen & AUDIO_INPUT_FLAG_FAST))
                && (flags & AUDIO_INPUT_FLAG_FAST)) {
                reuseInputFlag = false;
            }
            ALOGD("New flags 0x%x, Old flags 0x%x, reuseInputFlag %d", flags, activeNonIdleOnNonSTProfileInputs[0]->flagsToOpen, reuseInputFlag);
            if (reuseInputFlag) {
    ...
    if (isSoundTrigger) {
        ...
        if (nonSTProfileInputs.size()) {
            // ALPS04927537
            ALOGD("nonSTProfileInputs size=%zu curInputFlag 0x%x reqFlag 0x%x curRate %d reqRate %d", nonSTProfileInputs.size(),
                nonSTProfileInputs[0]->flagsToOpen, flags, nonSTProfileInputs[0]->mSamplingRate, config->sample_rate);

            if ((strncmp(profile->getTagName().string(), "primary input", strlen("primary input")) == 0)) {
                if (((nonSTProfileInputs[0]->flagsToOpen & AUDIO_INPUT_FLAG_FAST) && !(flags & AUDIO_INPUT_FLAG_FAST))) {
                    ALOGD("Reuse Flag inputId=%d", nonSTProfileInputs[0]->mIoHandle);
                    return nonSTProfileInputs[0]->mIoHandle;
                } else if (config->sample_rate <= nonSTProfileInputs[0]->mSamplingRate) {
                    ALOGD("Reuse Rate inputId=%d", nonSTProfileInputs[0]->mIoHandle);
                    return nonSTProfileInputs[0]->mIoHandle;
                }
            }
            ALOGW("%s #%d", __FUNCTION__, __LINE__);
